...
# The sequence can start with a blank token or the first character token and end with a blank token or the last character token.
# # So we have to consider both paths.
    ...
    for t in range(1, T):
        for c in range(C):
            if c == 0:
                ...
                alpha[t, c] = alpha[t-1, c] + \
                    alpha[t-1, c-1] + inputs[t, labels[c]]
    ...
    for t in range(T-2, -1, -1):
        for c in range(C-1, -1, -1):
            if c == C-1:
                ...
                beta[t, c] = beta[t+1, c] + \
                    beta[t+1, c+1] + inputs[t, labels[c]]
...
# Convert the output probabilities back to alphabet labels
